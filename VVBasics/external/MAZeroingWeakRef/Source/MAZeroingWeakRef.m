...
@interface NSObject (VV_MAZeroingWeakRefSwizzled)
- (void)VV_MAZeroingWeakRef_KVO_original_release;
- (void)VV_MAZeroingWeakRef_KVO_original_dealloc;
- (void)VV_MAZeroingWeakRef_KVO_original_addObserver:(NSObject *)observer forKeyPath:(NSString *)keyPath context:(void *)context;
- (void)VV_MAZeroingWeakRef_KVO_original_removeObserver:(NSObject *)observer forKeyPath:(NSString *)keyPath;
- (void)VV_MAZeroingWeakRef_KVO_original_removeObserver:(NSObject *)observer forKeyPath:(NSString *)keyPath context:(void *)context;
@end


static void EnsureCustomSubclass(id obj);

@interface VV_MAZeroingWeakRef ()
...
@interface _VV_MAZeroingWeakRefCleanupHelper : NSObject
{
    VV_MAZeroingWeakRef *_ref;
    id _target;
}

- (id)initWithRef: (VV_MAZeroingWeakRef *)ref target: (id)target;

@end

@implementation _VV_MAZeroingWeakRefCleanupHelper
...
- (void)dealloc
{
    VV_MAZeroingWeakRef *ref = objc_loadWeak_fptr(&_ref);
...
@implementation VV_MAZeroingWeakRef
...
+ (void)initialize
{
    if(self == [VV_MAZeroingWeakRef class])
    {
...
static void AddWeakRefToObject(id obj, VV_MAZeroingWeakRef *ref)
{
#if __APPLE__
...
static void RemoveWeakRefFromObject(id obj, VV_MAZeroingWeakRef *ref)
{
#if __APPLE__
    CFMutableSetRef set = (void *)CFDictionaryGetValue(gObjectWeakRefsMap, obj);
...
{
    IMP originalRelease = class_getMethodImplementation(object_getClass(self), @selector(VV_MAZeroingWeakRef_KVO_original_release));
    WhileLocked({
...
{
    ClearWeakRefsForObject(self);
    IMP originalDealloc = class_getMethodImplementation(object_getClass(self), @selector(VV_MAZeroingWeakRef_KVO_original_dealloc));
...
{
    WhileLocked({
        IMP originalIMP = class_getMethodImplementation(object_getClass(self), @selector(VV_MAZeroingWeakRef_KVO_original_removeObserver:forKeyPath:));
...
{
    WhileLocked({
        IMP originalIMP = class_getMethodImplementation(object_getClass(self), @selector(VV_MAZeroingWeakRef_KVO_original_removeObserver:forKeyPath:context:));
...
static void CallCFReleaseLater(CFTypeRef cf)
{
    ...
    NSInvocation *inv = [NSInvocation invocationWithMethodSignature: [VV_MAZeroingWeakRef methodSignatureForSelector: sel]];
    [inv setTarget: [VV_MAZeroingWeakRef class]];
    [inv setSelector: sel];
...
static BOOL CanNativeZWRClass(Class c)
{
    ...
    if(HashPresentInTable(hash, CC_SHA1_DIGEST_LENGTH, _VV_MAZeroingWeakRefClassNativeWeakReferenceNotAllowedTable))
        return NO;
    else
...
{
    NSString *newName = [NSString stringWithFormat: @"%s_VV_MAZeroingWeakRefSubclass", class_getName(class)];
...
//    NSLog(@"Patching KVO class %s", class_getName(class));
    ...
    class_addMethod(class,
                    @selector(VV_MAZeroingWeakRef_KVO_original_removeObserver:forKeyPath:),
                    method_getImplementation(removeObserverForKeyPath),
                    method_getTypeEncoding(removeObserverForKeyPath));
    class_addMethod(class, @selector(VV_MAZeroingWeakRef_KVO_original_release), method_getImplementation(release), method_getTypeEncoding(release));
    class_addMethod(class, @selector(VV_MAZeroingWeakRef_KVO_original_dealloc), method_getImplementation(dealloc), method_getTypeEncoding(dealloc));
    ...
    {
        class_addMethod(class,
                        @selector(VV_MAZeroingWeakRef_KVO_original_removeObserver:forKeyPath:context:),
...
static void RegisterRef(VV_MAZeroingWeakRef *ref, id target)
{
    WhileLocked({
...
static void UnregisterRef(VV_MAZeroingWeakRef *ref)
{
    WhileLocked({
...
- (void)setCleanupBlock: (void (^)(id target))block
{
    ...
    if(objc_loadWeak_fptr && _nativeZWR)
    {
        ...
        if(target != nil) @synchronized(target)
        {
            ...
            _VV_MAZeroingWeakRefCleanupHelper *helper = [[_VV_MAZeroingWeakRefCleanupHelper alloc] initWithRef: self target: target];

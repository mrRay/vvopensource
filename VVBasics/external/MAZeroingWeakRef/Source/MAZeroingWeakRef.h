...
@interface VV_MAZeroingWeakRef : NSObject
{
    id _target;
    BOOL _nativeZWR;
...
#define MAWeakDeclare(var)        __typeof__((var)) MAWeakVar(var) = (id)[VV_MAZeroingWeakRef refWithTarget:var]
#define MAWeakImport(var)         __typeof__((MAWeakVar(var))) var = [(VV_MAZeroingWeakRef *)MAWeakVar(var) target]
#define MAWeakImportReturn(var)   MAWeakImport(var); do { if(var == nil) return; } while(NO)
